...
| 5   | 0:54-1:14 | **The receipt beat.** Hold the evidence panel: two **Served Model** names, two different `Request Id` values, both `Receipt` fields reading `Verified`. _(Sample record, read 3 September)_ | "To review a question, Cekgu sends its wording and options — without the supplied key — through GonkaRouter to two distinct model families. The educator can inspect each reading, the served model, the gateway request ID, and its receipt status." |
...
**Recording notes, carried over from the original draft.**

- **Shot 5 shows the sample record read on 3 September, still live at the gateway.** The two readers named on screen
  there, MiniMax and Kimi, are the ones the gateway served that day; it delisted Kimi on 5 September. A re-shoot should
  name the seats rather than the pair, or put the date on screen. If a live check is filmed today the second reader is
  `deepseek-ai/DeepSeek-V4-Flash-0731`, so say "the second reader" rather than the name and avoid a contradiction with
  the recorded sample.
