...
| 5   | 0:54-1:14 | **The receipt beat.** Hold the evidence panel: **Reader A** `moonshotai/Kimi-K2.6`, **Reader B** `MiniMaxAI/MiniMax-M2.7`, two different `Request Id` values, both `Receipt` fields reading `Verified`                                                                | "To review a question, Cekgu sends its wording and options — without the supplied key — through GonkaRouter to two distinct model families. The educator can inspect each reading, the served model, the gateway request ID, and its receipt status." |
