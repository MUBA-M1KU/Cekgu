...
1. **The public web is consulted, and quoted.** Before the readers run, one search fetches up to four pages relevant to
   ...
   is hard-coded false — a provider's own generated answer would be reasoning off the gateway — and both that flag and
   the absence of any `answer` read are asserted by tests. Without `TAVILY_API_KEY` the readers work from their own
   ...
   **Every record states which of three retrieval states it is in:**
   ...
   | Pages attached **after** the readings | Labelled as fetched later — the readers did not see them, and no verdict or score uses them |
...
- Every attempt is recorded, admitted or refused — a timeout, a 429, a receipt mismatch and a fallback each leave a row
  with its own reason, shown in the evidence panel under the two readings.
...
- A dashboard counting verified readings against total for the account, and each family's share of the work by served
  model.
- A pasted link, fetched and reduced to the words on the page by a parser that calls no model, then structured into a
  draft by a Gonka model — the only input that reaches nothing outside the Gonka network, so it works with no vision
  key. Links resolving inside a private network are refused before a socket opens, which matters on Cloud Run where
  `169.254.169.254` hands out service-account tokens.
- Upload of PNG, JPEG, WebP or PDF up to 10 MB, transcribed to text and then structured into a draft by a Gonka model.
- An optional Live2D mascot, off unless `MASCOT_ENABLED=true`, respecting Reduce Motion and falling back to a still
...
- It may not adjudicate — no naming a correct option, confirming or rejecting a key, or solving a question. Asked "why
  is question 1 flagged?" on a **Clear** question, it says so instead of inventing a reason
- The off-gateway path survives behind `CHAT_PROVIDER=gemini`; if used, the turn's id is labelled by provider and drawn
  as a visibly different object from a Gonka receipt — unlinked, because there is nothing to open
