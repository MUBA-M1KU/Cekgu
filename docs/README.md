...
<p>
  ...
  Two AI models read each question blind, before your learners do. Each admitted reading shows its Gonka request id,
  and you can inspect the gateway receipt yourself.
</p>
...
Demo video: pending [#47](https://github.com/MUBA-M1KU/Cekgu/issues/47). The X handle is `@Cekgu0903`.
...
The five verdicts are evaluated in this order; the first matching condition wins:
...
| **Split Opinion**      | The two readers chose different options                               | Inspect the disagreement and use subject expertise        |
| **Possible Ambiguity** | Readers agree on an answer, but both list multiple defensible options | Review the wording and options                            |
| **Clear**              | Earlier rules did not fire and both readers chose the key             | No disagreement flagged. Not a certificate of correctness |
| **Possible Key Error** | Earlier rules did not fire and both chose the same non-key option     | Review the supplied key                                   |
...
The interface is English only for the submission. The question-language field is not an interface-language switch;
additional interface locales remain future work.
...
one family twice, and nothing in the response body would say so. Distinctness is therefore checked against the serving
model reported in the receipt, not inferred from which model was asked for.
...
public/                  brand, hero media, static mascot images and Live2D model files
drizzle/                 five committed SQL migrations
e2e/                     Playwright smoke, demo-path and opt-in live-flow tests
docs/                    this documentation tree, and the GitHub-facing readme
  legal/                 non-final demo terms, privacy and acceptable use; owner review required
...
The configured reader families are `deepseek-ai/DeepSeek-V4-Flash-0731`, `MiniMaxAI/MiniMax-M2.7` and
`moonshotai/Kimi-K2.6`. Two readings count as distinct only when their receipts name different models, so the third
family exists for when one of the other two is failing. These are committed constants, not a model list dynamically
validated against `GET /v1/models` at startup.
...
Open `http://localhost:5173`; Vite proxies `/api/` to `8080`. For a production-style local run, set
`BETTER_AUTH_URL=http://localhost:8080`, run `bun run build` then `bun run start`, and open that origin. Google OAuth is
optional and requires both Google credentials plus matching redirect configuration. Email/password and Guest do not
require Google. `MASCOT_ENABLED` controls animated stages, not the static mascot images on the landing page.
...
| `bun run dev`           | API and Vite development servers   |
| `bun run build`         | Build the browser app              |
| `bun run start`         | API, worker and built static app   |
| `bun test`              | Unit tests                         |
...
`bun run e2e` tests a **running URL**; it does not start a server. It defaults to production and takes a preview or a
running local instance through `E2E_BASE_URL`:

```bash
bun run e2e                                                        # production
bun run e2e:local                                                 # local server on 8080
```
...
- **The receipt reports which model served a request, not that the reasoning was sound.** It is gateway metadata, public
...
- **The Guest workspace is shared, not anonymous.** Anything typed there can be read or deleted by another guest, and
  Guest records are scheduled to expire after 24 hours, with removal dependent on successful worker cleanup
- **Confidential papers and personal data are prohibited**, including in private accounts. There are no dedicated
  learner fields, but free text can still store personal data if entered. Cekgu reviews the paper, never the cohort
...
- Draft demo notices: [terms](legal/terms.md), [privacy](legal/privacy.md), and
  [acceptable use](legal/acceptable-use.md). Not final; owner approval and deployed-product re-review are required
  before publication
