...
describe('demo recording beats', () => {
  ...
  test(
    'targets the reader-column model when the same value repeats in receipts and attempts',
    async () => {
  ...
  test(
    'frames evidence below the persistent top bar',
    async () => {
      const { chromium } = await import('playwright')
      const browser = await chromium.launch({ channel: 'chrome', headless: true })
  ...
  test(
    'targets the attempt-table status when its reason repeats the label',
    async () => {
      const { chromium } = await import('playwright')
      const browser = await chromium.launch({ channel: 'chrome', headless: true })
      const page = await browser.newPage()
  ...
  test(
    'does not mistake a recorded disposition chip for an editable decision',
    async () => {
      const { chromium } = await import('playwright')
      const browser = await chromium.launch({ channel: 'chrome', headless: true })
      const page = await browser.newPage()
  ...
  test(
    'resets the sample through the current browser session',
    async () => {
      const { chromium } = await import('playwright')
      const browser = await chromium.launch({ channel: 'chrome', headless: true })
      const page = await browser.newPage()
