...
// Prose, not a workspace: one column held to a reading measure. These three are the only public
// routes that are documents rather than tools.
...
export function PublicLayout() {
  ...
  const notice = NOTICES.includes(pathname)
  // A receipt is a document too for this purpose: short enough that an in-flow footer sits on the
  // floor of the first screen.
  const revealFooter = notice || pathname.startsWith('/receipt/')
  const measure = notice ? 'max-w-[44rem]' : 'max-w-[76rem]'
  ...
  return (
    ...
    <div className="flex min-h-dvh flex-col bg-paper">
      {/* display: contents where the footer stays in flow, so the header and the main below remain
          the outer column's own flex children and its floor behaviour is untouched. Where the
          footer is revealed instead, this is the opaque column that hides it. */}
      <div className={revealFooter ? 'public-doc' : 'contents'}>
        ...
        <main className={overHero ? 'grow' : `mx-auto w-full grow px-4 py-6 sm:px-6 ${measure}`}>
          <Outlet />
        </main>
      </div>
      ...
      <footer className="public-footer shrink-0" role="contentinfo" data-reveal={revealFooter ? 'true' : undefined}>
        <SiteFooter />
      </footer>
