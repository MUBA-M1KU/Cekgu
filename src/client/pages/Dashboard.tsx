...
export function Dashboard() {
  ...
  return (
    <Sheet>
      ...
      {failed ? (
        <p className="type-body mt-6 text-ink-muted">We could not reach your records, try again in a moment.</p>
      ) : null}
          ...
          {recent.map((record) => (
            <li key={record.id} className="border-t border-rule">
              <Link to={`/records/${record.id}`} className="flex flex-wrap items-baseline gap-x-4 gap-y-1 py-3">
                <span className="type-body min-w-0 flex-1">{record.title}</span>
