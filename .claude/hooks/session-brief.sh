...
stage=$(sed -n 's/^stage:[[:space:]]*//p' docs/STAGE 2>/dev/null | head -1)
...
echo "MUBA/GonkaRouter | stage=${stage:-unknown} | branch=${branch:-?} | uncommitted=${dirty:-0} | ${left}d to submission"
